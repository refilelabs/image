...
import type { Metadata, MetadataPresets } from '#image/wasm/pkg/web/refilelabs_image'
...
import { deletableFields, editableFields, metadataPresets } from '#image/wasm/pkg/web/refilelabs_image'
...
const removeMenuItems = computed(() => [[
  ...
  },
  {
    ...
    type: 'checkbox' as const,
    checked: (presets.value?.timestamps ?? []).length > 0 && (presets.value?.timestamps ?? []).every(f => deletions.value.has(f)),
  ...
  },
  {
    ...
    type: 'checkbox' as const,
    checked: (presets.value?.device ?? []).length > 0 && (presets.value?.device ?? []).every(f => deletions.value.has(f)),
  ...
  },
  {
    ...
    icon: 'heroicons:user',
    type: 'checkbox' as const,
    checked: (presets.value?.author ?? []).length > 0 && (presets.value?.author ?? []).every(f => deletions.value.has(f)),
